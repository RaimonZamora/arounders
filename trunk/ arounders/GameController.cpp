...
bool GameController::Init()
{
	...
	if ( ((gameInfo->fase+1) % 5) == 0) {
		musicManager->Load( "mus6.mid");
	} else {
		musicManager->Load( "mus4.mid");
	}
...
void GameController::Go(GameInfo *pGameInfo)
{
	...
	while ( !salir ) {
		...
		if (inputManager->Pausa() || inputManager->BotoPulsat(SDL_BUTTON_MIDDLE)) {
